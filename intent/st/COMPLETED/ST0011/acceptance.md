...
- AT-07.3 test/arca_cli/dead_code_gate_test.exs::"invariant: the scanner finds strings that are genuinely present" -- covers AC-07.1 -- status: green. It covers the GATE itself and is AC-07.1's positive control rather than another instance of it: without this row, a scanner that silently matched nothing would report every invariant above as satisfied.
...
- AT-11.6 test/arca_cli/history/degradation_test.exs, describe "sys.flush with History down" (3 tests) -- covers AC-11.1 -- status: green. Lives beside the unregister seam that makes it possible rather than with the other A13 coverage, because a failure branch is only covered if something can drive it. Added after vc proved the first A24 fix inert.
