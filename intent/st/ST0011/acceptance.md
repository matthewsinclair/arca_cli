...
- AC-11.1 (test) The last four A13-class paths return failure through the outcome channel rather than as display text: `sys.flush` (A24), `cfg.list` (A19), `Arca.Cli.Command.BaseSubCommand` (A18) and `Arca.Cli.Configurator.Coordinator` (A20, both `inject_subcommands/2` and the silently-skipped command in `update_command_names/3`) -- evidence: AT-11.1, AT-11.2 -- satisfied: yes. Coverage is honest about its own shape: A18 and A20 are driven behaviourally because their failure branches are reachable from outside; A19 and A24 fail only when a dependency they call fails and have no seam to drive that without mocking our own modules (IN-EX-TEST-006), so they are covered by the construct gate plus the dispatch contract already proven in AC-01.1.
- AC-11.2 (test) Ctx-reported failures reach the user in every text style, in the ratified dialect: both `:ansi` and `:plain` emit a line matching `^error:` in the form `error: <command>: <message>` alongside the existing `✗` block; `:json` stays structured and unchanged -- evidence: AT-11.3, AT-11.4 -- satisfied: yes. Implementing this surfaced finding A25, which is larger than the dialect gap it was scoped for: the `:ansi` renderer read `ctx.output` and nothing else, so an error-only context rendered to the empty string. hv's ruling was "both"; the answer turned out to be "one of them was rendering nothing at all".
